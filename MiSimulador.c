...
directa cache[128]; // Vector que simula la ocupaci— de mem cache de 128 linies ja que 128*32Byte = 4KB guardarem els tag.
...
void reference (unsigned int address)
{
    ...
    if (cache[linea_mc].validez == 0) { //Miss lectura, linia de cache buida (no reemplaament)
        cache[linea_mc].tag = tag;
        cache[linea_mc].validez = 1;
    }
    else if (cache[linea_mc].validez == 1 && cache[linea_mc].tag != tag) { //Miss lectura, (si reemplaament)
